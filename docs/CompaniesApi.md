...
async with financial_reports_generated_client.ApiClient(configuration) as api_client:
    ...
    date_public_after = '2013-10-20' # date | Companies last updated on or after this date (YYYY-MM-DD). Granularity is one day, a Europe/Berlin calendar date. NOTE: roughly 790 listed companies have no date_public yet and a range filter cannot return them, so seed the catalogue once before relying on incremental syncs — and partition that first pass (by `countries=`, say), because a single unfiltered walk stops at the 50,000-row offset cap well short of the ~95,000 companies. ALSO: date_public changes whenever a company is updated, including while you are paging, pages are offset-based, and the total count behind `next` is cached — so a walk that spans updates can repeat companies or miss some. Treat incremental sync on this field as best-effort: page in `-date_public` order (a company updated mid-walk is then re-dated into your NEXT window rather than lost), de-duplicate by `id`, start each window at least a day before the previous one ended, and re-seed periodically. (optional)
    date_public_before = '2013-10-20' # date | Companies last updated on or before this date (YYYY-MM-DD). Granularity is one day, a Europe/Berlin calendar date. NOTE: roughly 790 listed companies have no date_public yet and a range filter cannot return them, so seed the catalogue once before relying on incremental syncs — and partition that first pass (by `countries=`, say), because a single unfiltered walk stops at the 50,000-row offset cap well short of the ~95,000 companies. ALSO: date_public changes whenever a company is updated, including while you are paging, pages are offset-based, and the total count behind `next` is cached — so a walk that spans updates can repeat companies or miss some. Treat incremental sync on this field as best-effort: page in `-date_public` order (a company updated mid-walk is then re-dated into your NEXT window rather than lost), de-duplicate by `id`, start each window at least a day before the previous one ended, and re-seed periodically. (optional)
