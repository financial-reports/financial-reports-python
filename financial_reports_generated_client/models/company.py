...
class Company(BaseModel):
    ...
    listing_status: StrictStr = Field(description="Canonical exchange-listing state. One of: LISTED, DELISTED, SUSPENDED, UNKNOWN. The boolean `is_listed` is derived from this (False only when DELISTED).")
    delisting_date: Optional[date] = Field(description="Date the company was delisted, if known. Null while listed.")
    delisting_reason: StrictStr = Field(description="Reason for delisting when DELISTED: ACQUISITION, BANKRUPTCY, GOING_PRIVATE, REGULATORY, VOLUNTARY, EXCHANGE_TRANSFER, MATURED, OTHER. Empty string while listed.")
    ...
    listings: List[CompanyListing] = Field(description="Equity venue listings for this company (exchange + ticker), deduplicated by (MIC, ticker). Excludes warrants/options/futures/rights. Populated from per-venue identifier data; coverage expands over time.")
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * OpenAPI `readOnly` fields are excluded.
        ...
        # set to None if delisting_date (nullable) is None
        # and model_fields_set contains the field
        if self.delisting_date is None and "delisting_date" in self.model_fields_set:
            _dict['delisting_date'] = None
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "delisting_date": obj.get("delisting_date"),
            "delisting_reason": obj.get("delisting_reason"),
            ...
            "listings": [CompanyListing.from_dict(_item) for _item in obj["listings"]] if obj.get("listings") is not None else None,
