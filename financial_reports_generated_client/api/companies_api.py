...
class CompaniesApi:
    ...
    async def companies_financials_retrieve(
        ...
        as_of: Annotated[Optional[StrictStr], Field(description="Filing-date cutoff (`YYYY-MM-DD`): only filings released on or before `as_of` are considered when picking the statement for each period. This is NOT a snapshot of the values we served on that date - figures are corrected in place, and a request with `as_of` returns today's version of each value from those filings. Use each line item's `updated_at` to see when a value last changed.")] = None,
        ...
        **Experimental endpoint — the response schema may change without an API version bump.**  Returns standardized financial KPIs for a company as a structured document: a company envelope containing `periods`, each holding its Income Statement, Balance Sheet and Cash Flow `statements`, each holding `line_items`.  When several filings report the same period, the candidates are ranked and one statement is selected. A filing the company reports its own financials in beats one it does not, such as an employee benefit plan's Form 11-K or a filing notice. A statement in the currency the company reports in beats an off-currency one, which is usually a subsidiary's filing that resolved to the parent. A complete statement beats a fragment, such as the single restated line in an amendment. Only then does the most recently published filing win. A selected statement denominated in a currency other than the company's carries `currency_mismatch: true`.  **Currency is as presented, and is never converted.** `currency` is the currency the statement is denominated in as the filer presented it - the presentation currency, not the functional currency, which we do not capture. No figure in this payload is ever translated into another currency at any rate, and `value` being in absolute units makes values comparable across scales, not across currencies. A company that re-presents in a new currency therefore produces a series whose units change partway through, with no restatement of the earlier years: compare `currency.code` across periods before aggregating or charting. A line item may carry its own `currency` differing from the statement's - that is a dual-currency filing, not an error.  Each statement carries `source_filing`: the filing the figures were read from, with `document_url` and `viewer_url` pointing at our hosted copy of that document. This is returned to every account. The publishing authority's identity and its own direct URL remain masked unless the account has source unmasking enabled. The full `sources` array — every contributing filing, including candidates that selection rejected — is returned only with source unmasking (`sources_masked` reports which applies). `as_of=YYYY-MM-DD` restricts selection to filings released on or before that date; it does not reproduce the values served on that date, because figures are corrected in place. Each line item's `updated_at` says when its value was last written.  Use the `depth` and `parent_code` fields on each line item to render the Capital IQ-style statement hierarchy.  `value` and `raw_value` are exact **decimal strings**, not numbers - parse them with a decimal type. `raw_value` and `scale` come back `null` when the figure was computed rather than read off the page. `confidence` and `source_page` are reserved and are currently `null` on every line item; provenance is filing-level.  **Scope - this is not the bulk dataset.** This endpoint serves one selected statement per (fiscal year, period, statement type). The comparative statements themselves, restatements, the `validation_n_hard`/`validation_n_soft` quality flags, `derivation_method` and stable row ids are available only in the S3 `line_items/` parquet delivery, not here at any plan level. The `is_comparative` flag on each statement does tell you whether the selected figures were read from a later report's prior-year column.  **Cost:** 40 credits per request, flat - independent of how many periods or line items are returned, and not reduced by filtering. Pay-as-you-go accounts are limited to a rolling window on filing release date - 730 days by DEFAULT, widenable per account - and when that applies the response carries a `history_window` object. Read the effective limit from `history_window.max_history_days` rather than assuming 730. The deep back-catalogue requires an unfenced plan.  **Access Level Required:** Requires **Financial KPIs (Level 3)**.

        :param id: A unique integer value identifying this company. (required)
        :type id: int
        :param as_of: Filing-date cutoff (`YYYY-MM-DD`): only filings released on or before `as_of` are considered when picking the statement for each period. This is NOT a snapshot of the values we served on that date - figures are corrected in place, and a request with `as_of` returns today's version of each value from those filings. Use each line item's `updated_at` to see when a value last changed.
        :type as_of: str
    ...
    async def companies_list(
        ...
        **Access Level Required:** Requires **Standard Access (Level 1)**.  --- Retrieve a paginated list of companies.
        ...
        :type countries: str
        :param date_public_after: Companies last updated on or after this date (YYYY-MM-DD). Granularity is one day, a Europe/Berlin calendar date. NOTE: roughly 790 listed companies have no date_public yet and a range filter cannot return them, so seed the catalogue once before relying on incremental syncs — and partition that first pass (by `countries=`, say), because a single unfiltered walk stops at the 50,000-row offset cap well short of the ~95,000 companies. ALSO: date_public changes whenever a company is updated, including while you are paging, pages are offset-based, and the total count behind `next` is cached — so a walk that spans updates can repeat companies or miss some. Treat incremental sync on this field as best-effort: page in `-date_public` order (a company updated mid-walk is then re-dated into your NEXT window rather than lost), de-duplicate by `id`, start each window at least a day before the previous one ended, and re-seed periodically.
